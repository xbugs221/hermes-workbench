/**
 * 文件目的：把 Workbench 源码构建为 Hermes Dashboard 可直接加载的单 IIFE。
...
function dashboardSDKBoundary(): Plugin {
  return {
    ...
    resolveId(source, importer) {
      ...
      this.error(
        `Hermes Workbench must use sdk.React; forbidden runtime import "${source}"`
    ...
    generateBundle(_options, bundle) {
      ...
      if (chunks.length !== 1) {
        this.error(`Hermes Workbench must emit one IIFE chunk; received ${chunks.length}`);
      }

      const [chunk] = chunks;
      if (chunk.dynamicImports.length > 0 || chunk.isDynamicEntry) {
        this.error('Hermes Workbench must not emit dynamic imports or dynamic entry chunks');
      }

      const fingerprint = EMBEDDED_REACT_FINGERPRINTS.find(value => chunk.code.includes(value));
      if (fingerprint) {
        this.error(`Hermes Workbench bundle contains an embedded React fingerprint: ${fingerprint}`);
      }
      ...
      if (rawBytes > MAX_IIFE_BYTES || gzipBytes > MAX_IIFE_GZIP_BYTES) {
        this.error(
          `Hermes Workbench bundle exceeds budget: ${rawBytes} raw bytes / ${gzipBytes} gzip bytes `
...
/** Ship Mermaid as a same-origin lazy asset instead of inflating the main IIFE. */
function selfHostedMermaid(): Plugin {
  return {
    name: 'self-hosted-mermaid-runtime',
    apply: 'build',
    buildStart() {
      this.emitFile({
        type: 'asset',
        fileName: 'mermaid.min.js',
        source: readFileSync(path.resolve(integrationRoot, 'node_modules/mermaid/dist/mermaid.min.js')),
      });
    },
  };
}
...
export default defineConfig({
  // 独立插件只发布自身 bundle，不复制任何宿主应用的静态资源。
  publicDir: false,
  plugins: [dashboardSDKBoundary(), selfHostedMermaid()],
  build: {
