 ...
 */
import type { HermesDisplayEvent, HermesTurn } from './hermes-transcript';
import type { ChatMessage } from './chat-display/types';
import { micromark } from 'micromark';
import { gfm, gfmHtml } from 'micromark-extension-gfm';
import {
  buildTurnDisplayBlocks,
  type TurnNonBodyGroupBlock,
} from './chat-display/turn-non-body-collapse';
import { formatProcessedDuration } from './chat-display/duration';
...
const EARLIER_TURN_PAGE_SIZE = 20;
...
export function createTranscriptComponents(sdk: SDK): { TranscriptTimeline: (props: { turns: HermesTurn[]; rawOpen: boolean }) => any } {
  ...
  /** 首屏只渲染尾部 turns；用户按需向前扩展，避免长记录阻塞 Dashboard。 */
  function TranscriptTimeline({ turns, rawOpen }: { turns: HermesTurn[]; rawOpen: boolean }): any {
    const [visibleCount, setVisibleCount] = hooks.useState(INITIAL_VISIBLE_TURNS);
    const firstVisibleIndex = Math.max(0, turns.length - visibleCount);
    const visibleTurns = turns.slice(firstVisibleIndex);
    return element(React, 'div', {
      className: 'hti-timeline',
      'data-testid': 'hermes-workbench-timeline',
      'data-component': 'TranscriptTimeline',
    },
    firstVisibleIndex > 0 ? element(React, 'button', {
      type: 'button',
      className: 'hti-load-earlier',
      'data-testid': 'load-earlier-turns',
      onClick: () => setVisibleCount((current: number) => Math.min(
        turns.length,
        current + EARLIER_TURN_PAGE_SIZE,
      )),
    }, '加载更早 / Load earlier') : null,
    ...visibleTurns.map((turn, index) => element(React, TranscriptTurn, {
      React,
      hooks,
      turn,
      rawOpen,
